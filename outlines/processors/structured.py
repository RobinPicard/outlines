...
if TYPE_CHECKING:
    ...
    import torch  # Add this import for type hints
...
class GuideLogitsProcessor(OutlinesLogitsProcessor):
    ...
    def process_logits(
        ...
        for seq_ids in input_ids: # type: ignore
            ...
            curr_state_key = hash(tuple(self.tensor_handler.to_list(gen_ids)))

            if curr_state_key not in self._guide_states:
                prev_state = self._guide_states[hash(tuple(self.tensor_handler.to_list(gen_ids[:-1])))]
                curr_state = self.guide.get_next_state(prev_state, self.tensor_handler.to_scalar(gen_ids[-1]))
        ...
        for i, guide_state in enumerate(sequence_states):
            ...
            batch_indices.append(
                self.tensor_handler.full_like(allowed_tokens, i)
            )  # Store batch index for each allowed token

        device = self.tensor_handler.get_device(logits)
        allowed_tokens_concat = self.tensor_handler.to_device(
            self.tensor_handler.concatenate(allowed_tokens_batch),
            device
        )
        batch_indices_concat = self.tensor_handler.to_device(
            self.tensor_handler.concatenate(batch_indices),
            device
        )

        mask = self.tensor_handler.boolean_ones_like(logits)
        mask[batch_indices_concat, allowed_tokens_concat] = False
        logits = self.tensor_handler.apply_mask(logits, mask, float("-inf"))

        return logits

    def copy(self) -> "GuideLogitsProcessor":
        """Return a copy of the logits processor."""
        return GuideLogitsProcessor(tokenizer=self.tokenizer, guide=self.guide.copy(), tensor_library_name=self.tensor_handler.library_name)
